...
crow::json::wvalue DbHandler::addRate(crow::json::rvalue x) {
    string giorni;  //Stringa per i giorni in cui la nuova tariffa sarà valida
    for (int i = 0; i < x["Giorno"].size(); i++) {
        res = stmt->executeQuery("SELECT id FROM tariffa WHERE giorni LIKE '%" + (std::string)x["Giorno"][i].s() + "%'"); //Controllo non ci siano tariffe che valgono per gli stessi giorni
        giorni += (std::string)x["Giorno"][i].s() + " ";
    }
    if (res->next()) {
        throw std::bad_exception();
    }
    pstmt = con->prepareStatement("INSERT INTO tariffa(val_fisso, aggiunta_ore, giorni) VALUES(?,?,?)");
    pstmt->setDouble(1, (double)x["Valore_fisso"].d());
    pstmt->setDouble(2, (double)x["Aggiunta_ore"].d());
    pstmt->setString(3, giorni);
    pstmt->execute();
    crow::json::wvalue s;
    s["Result"] = "Successo";
    return s;
...
crow::json::wvalue DbHandler::showRate() {
    int i = 0;
    res = stmt->executeQuery("SELECT * FROM tariffa");
    crow::json::wvalue tariffe;
    res->first();  //riporto il result set al primo valore
    do {
...
crow::json::wvalue DbHandler::updateRate(crow::json::rvalue x) {
    crow::json::wvalue result;
    string giorni;
    for (int i = 0; i < x["Giorno"].size(); i++) {
        res = stmt->executeQuery("SELECT id FROM tariffa WHERE giorni LIKE '%" + (std::string)x["Giorno"][i].s() + "%'"); //Controllo non ci siano tariffe che valgono per gli stessi giorni
