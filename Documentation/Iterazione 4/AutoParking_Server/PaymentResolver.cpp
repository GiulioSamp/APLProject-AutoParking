...
PaymentResolver& PaymentResolver::getInstance() {  //Classe singleton
...
void PaymentResolver::aggiorna_guadagni(double x) { //Metodo per test
	guadagni_totali = x;
	guadagni_mensili += x;
	guadagni_giornalieri += x;
}

std::vector<double> PaymentResolver::get_guadagni() { //Metodo per test
